...
@section('master')
    <div class="col-12">
        <table class="table table-borderless table--striped table--responsive--xl">
            ...
            <tbody>
                @forelse ($disputedJobs as $dispute)
                    <tr>
                        <td>
                            <div>
                                <p class="fw-semibold">{{ strlen($dispute->job->title) > 25 ? __(strLimit($dispute->job->title, 25)) : __($dispute->job->title) }}</p>
                                <p class="text--base">{{ $dispute->job->job_code }}</p>
                            </div>
                        </td>
                        <td>
                            <div>
                        ...
                        <td>
                            <a href="{{ route('admin.job.dispute.show', [$dispute->job, $dispute->id]) }}" class="btn btn--sm btn-outline--base">
