...
@section('content')
    <section class="account">
        <div class="account__bg bg-img" data-background-image="{{ getImage($activeThemeTrue . 'images/site/mobile_confirm/' . @$mobileConfirmContent->data_info->background_image, '1920x1080') }}"></div>
        <div class="account__form">
            @include($activeTheme . 'partials.basicAccountTop')

            <div class="account__form__content">
                <h3 class="account__form__title">{{ __(@$mobileConfirmContent->data_info->form_heading) }}</h3>
            </div>
            <p class="mb-4 text-center">@lang('A six-digit verification code has been sent to') <b>{{ '+' . showMobileNumber($user->mobile) }}</b></p>
